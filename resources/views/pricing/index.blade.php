        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
            @forelse($packages as $package)
                ...
                <div class="bg-[#0a0a0a] border border-white/5 rounded-3xl p-8 shadow-2xl relative overflow-hidden group transition-all flex flex-col duration-300 hover:border-blue-500/30 hover:-translate-y-2">
                    ...
                    {{-- Button Form --}}
                    <form action="{{ route('pricing.select', $package->id) }}" method="POST" class="mt-auto" x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
                        @csrf
                        <button type="submit" 
                                :disabled="isSubmitting"
                                class="w-full py-4 text-[11px] font-black rounded-xl uppercase tracking-widest transition-all border {{ $buttonClass }} disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                            <svg x-show="isSubmitting" class="w-4 h-4 animate-spin text-current" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                            <span x-text="isSubmitting ? 'PROCESSING...' : '{{ $buttonText }}'"></span>
                        </button>
                    </form>
